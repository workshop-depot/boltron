...
func TestOpen(t *testing.T) {
	path := tempfile()
	db, err := Open(marshaler, path, 0666, nil)
	if err != nil {
		t.Fatal(err)
	} else if db == nil {
		t.Fatal("expected db")
...
func TestOpen_ErrInvalid(t *testing.T) {
	...
	if _, err := Open(marshaler, path, 0666, nil); err != bolt.ErrInvalid {
...
func TestOpen_ErrVersionMismatch(t *testing.T) {
	...
	// Reopen data file.
	if _, err := Open(marshaler, path, 0666, nil); err != bolt.ErrVersionMismatch {
		t.Fatalf("unexpected error: %s", err)
	}
}
...
func TestOpen_ErrChecksum(t *testing.T) {
	...
	// Reopen data file.
	if _, err := Open(marshaler, path, 0666, nil); err != bolt.ErrChecksum {
		t.Fatalf("unexpected error: %s", err)
	}
}
...
func TestOpen_Size(t *testing.T) {
	...
	db0, err := Open(marshaler, path, 0666, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDB_Open_InitialMmapSize(t *testing.T) {
	...
	db, err := Open(marshaler, path, 0666, &bolt.Options{InitialMmapSize: initMmapSize})
	if err != nil {
		t.Fatal(err)
	}
...
func ExampleDB_Update() {
	// Open the database.
	db, err := Open(marshaler, tempfile(), 0666, nil)
	if err != nil {
		log.Fatal(err)
	}
