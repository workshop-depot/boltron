...
func TestBucket_Put_IncompatibleValue(t *testing.T) {
	...
	if err := db.Update(func(tx *bolt.Tx) error {
		...
		if err := b0.Put([]byte("foo"), []byte("bar")); err != nil && err != bolt.ErrIncompatibleValue {
...
func TestBucket_Delete_Bucket(t *testing.T) {
	...
	if err := db.Update(func(tx *bolt.Tx) error {
		...
		if err := b.Delete([]byte("foo")); err != nil && err != bolt.ErrIncompatibleValue {
			t.Fatalf("unexpected error: %v", err)
		}
		return nil
	}); err != nil {
...
func TestBucket_CreateBucket_IncompatibleValue(t *testing.T) {
	...
	if err := db.Update(func(tx *bolt.Tx) error {
		...
		if _, err := widgets.CreateBucket([]byte("foo")); err != nil && err != bolt.ErrIncompatibleValue {
			t.Fatalf("unexpected error: %v", err)
		}
		return nil
	}); err != nil {
